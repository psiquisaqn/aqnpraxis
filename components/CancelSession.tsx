...
export function useCancelSession(sessionId: string, patientId?: string) {
  ...
  const cancel = () => {
    startTransition(async () => {
      ...
      if (patientId) router.push('/dashboard/paciente/' + patientId)
...
export function CancelSessionModal({ open, onConfirm, onClose, isPending }: ModalProps) {
  if (!open) return null
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-sm animate-fade-up rounded-2xl shadow-2xl overflow-hidden"
      ...
      >
        <div className="px-6 pt-6 pb-4 flex flex-col items-center text-center">
          <div className="w-12 h-12 rounded-full flex items-center justify-center mb-4" style={{ background: '#fef2f2' }}>
            <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
              <path d="M11 2L2 19h18L11 2z" stroke="#dc2626" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round"/>
              <path d="M11 9v4M11 15.5v.5" stroke="#dc2626" strokeWidth="1.75" strokeLinecap="round"/>
            </svg>
          </div>
          <h2 className="text-base font-semibold mb-1" style={{ color: 'var(--stone-800)', fontFamily: 'var(--font-serif)' }}>
        ...
        <div className="flex gap-2 px-6 py-4 border-t" style={{ borderColor: 'var(--stone-100)' }}>
          ...
          <button onClick={onConfirm} disabled={isPending}
            ...
            style={{ background: isPending ? 'var(--stone-300)' : '#dc2626', cursor: isPending ? 'not-allowed' : 'pointer' }}>
