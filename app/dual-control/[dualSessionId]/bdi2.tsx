...
export function Bdi2Control({ dualSessionId, sessionId, onUpdatePatient, onSaveResponse }: Bdi2ControlProps) {
  ...
  useEffect(() => {
    const handleDisplayReady = () => setDisplayReady(true)
  ...
  useEffect(() => {
    if (displayReady) sendCurrentItemToDisplay()
  ...
  const goToNext = () => { if (currentItem < 21) setCurrentItem(currentItem + 1) }
  const goToPrev = () => { if (currentItem > 1) setCurrentItem(currentItem - 1) }
  const goToItem = (num: number) => setCurrentItem(num)
  ...
  const handleFinish = async () => {
    ...
    for (let i = 1; i <= 21; i++) {
      if (responses[i] !== undefined) itemCols[`item_${i}`] = responses[i]
    }
    ...
    await supabase.from('dual_session_tests')
      .upsert({
