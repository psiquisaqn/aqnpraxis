  ...
  useEffect(() => {
    const init = async () => {
      try {
        let ms: MediaStream
        try {
          ms = await navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment', width: { ideal: 1920 }, height: { ideal: 1080 } } })
        } catch {
          ms = await navigator.mediaDevices.getUserMedia({ video: { facingMode: 'user', width: { ideal: 1920 }, height: { ideal: 1080 } } })
        }
        ...
        const noTrasera = !vdevs.some(d => {
          const dl = (d.label || '').toLowerCase()
          return dl.includes('back') || dl.includes('rear') || dl.includes('trasera')
        })
        const esFrontal = lab.includes('front') || lab.includes('user') || lab.includes('webcam') || lab.includes('integrated') || noTrasera
        console.log('📷 Camara detectada:', { label: dev?.label || track.label, isFront: esFrontal })
        setIsFront(esFrontal)
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-xl p-6 max-w-md w-full">
          ...
          <button onClick={onClose} className="w-full py-2 bg-gray-200 rounded-lg">Cerrar</button>
        </div>
      </div>
    )
  }
        ...
        {!preview ? (
          <>
            ...
            <button onClick={capture} disabled={!ready || capturing}
              className={'w-full py-3 rounded-lg font-medium ' + (!ready || capturing ? 'bg-gray-300 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700')}>
          ...
          <>
            ...
            <div className="flex gap-3">
              <button onClick={() => { onCapture(img!); if (stream) stream.getTracks().forEach(t => t.stop()) }}
                className="flex-1 py-3 bg-green-600 text-white rounded-lg font-medium">Usar esta foto</button>
...
// ============================================================
// PLANTILLA DE CORRECCION (CON GRID OFFSET DESDE SUPABASE)
// ============================================================
interface ScoringGridProps {
  imageData: string
  patientAge: number
  onScoreCalculated: (s: number, m: boolean[]) => void
  onClose: () => void
}
...
function ScoringGrid({ imageData, patientAge, onScoreCalculated, onClose }: ScoringGridProps) {
  ...
  // Cargar configuracion desde Supabase (AHORA CON GRID OFFSET)
  useEffect(() => {
    ...
    const loadGridConfig = async () => {
      ...
      if (error) { console.error('❌ Error al cargar configuracion:', error) }
      if (data) {
        console.log('✅ Datos recibidos correctamente')
        if (isA) {
          const nr = (data.template_a_row_positions as number[]) || rows
          const nrh = data.template_a_row_height || 0.10
          const nc = (data.template_a_cell_widths as number[]) || cws
          const ngx = data.template_a_grid_offset_x || 0
          const ngy = data.template_a_grid_offset_y || 0
          console.log('🅰️ Aplicando Plantilla A:', { rows: nr.length, rh: nrh, cws: nc.length, gridOff: { x: ngx, y: ngy } })
          setRows(nr); setRh(nrh); setCws(nc); setGridOff({ x: ngx, y: ngy })
        } else {
          const nr = (data.template_b_row_positions as number[]) || rows
          const nrh = data.template_b_row_height || 0.10
          const nc = (data.template_b_cell_widths as number[]) || cws
          const ngx = data.template_b_grid_offset_x || 0
          const ngy = data.template_b_grid_offset_y || 0
          console.log('🅱️ Aplicando Plantilla B:', { rows: nr.length, rh: nrh, cws: nc.length, gridOff: { x: ngx, y: ngy } })
          setRows(nr); setRh(nrh); setCws(nc); setGridOff({ x: ngx, y: ngy })
        }
      } else {
  ...
  useEffect(() => {
    ...
    const b = new Image(); b.src = imageData
    b.onload = () => { setBase(b); setPos({ x: b.width * 0.1, y: b.height * 0.1 }) }
  ...
  useEffect(() => {
    ...
    ctx.restore()
    if (show && overlay) {
      const sw = overlay.width * scale.x
      const sh = overlay.height * scale.y
      let idx = 0
      for (let p = 0; p < PAIRS; p++) {
        ...
        for (let col = 0; col < cp; col++) {
          ...
          const w = cws[ac] * sw
          let ax = 0
          for (let cc = 0; cc < ac; cc++) ax += cws[cc] * sw
          const x = pos.x + gridOff.x + ax
          const y = pos.y + gridOff.y + rows[p] * sh
          const h = rh * sh
          if (marks[idx]) {
  ...
  }, [base, overlay, pos, scale, marks, show, rows, rh, cws, gridOff, loaded])
  ...
  const down = (e: React.MouseEvent) => {
    if (e.shiftKey) { setDrag(true); setStart({ x: e.clientX - pos.x, y: e.clientY - pos.y }) }
  }
  const move = (e: React.MouseEvent) => {
    if (drag) setPos({ x: e.clientX - start.x, y: e.clientY - start.y })
  }
  ...
  if (!loaded) {
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white rounded-xl p-6">Cargando configuracion...</div>
      </div>
    )
  }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl p-4 max-w-6xl w-full max-h-screen overflow-auto">
        <h3 className="text-lg font-semibold mb-3">
          Plantilla Claves {isA ? 'A' : 'B'} ({isA ? '6-7' : '8-16'} anos) | Max: {CFG.maxScore}
        </h3>
        <div className="bg-blue-50 rounded-lg p-3 mb-3">
          <p className="text-xs text-blue-700">Clic en ventana azul = marcar ✓. Shift+Arrastrar = mover todo.</p>
        </div>
        <div className="overflow-auto border rounded-lg mb-3 bg-gray-100" style={{ maxHeight: '55vh' }}>
          <canvas ref={canvasRef} onClick={click} onMouseDown={down} onMouseMove={move} onMouseUp={up} onMouseLeave={up}
            className="cursor-crosshair" style={{ width: '100%', height: 'auto' }} />
        </div>
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs">Zoom X: {scale.x.toFixed(2)}</label>
              <input type="range" min="0.3" max="3" step="0.01" value={scale.x}
                onChange={e => setScale({ ...scale, x: +e.target.value })} className="w-full" />
            </div>
            <div>
              <label className="text-xs">Zoom Y: {scale.y.toFixed(2)}</label>
              <input type="range" min="0.3" max="3" step="0.01" value={scale.y}
                onChange={e => setScale({ ...scale, y: +e.target.value })} className="w-full" />
            </div>
          </div>
          <div className="flex gap-2 flex-wrap">
            ...
            <button onClick={() => setShow(!show)} className="px-3 py-1.5 bg-gray-100 rounded text-sm">
              {show ? 'Ocultar' : 'Mostrar'} rejilla
            </button>
          </div>
          <div className="flex gap-2">
            <button onClick={() => setMarks(new Array(total).fill(!marks.every(m => m)))}
              className="px-3 py-1.5 bg-gray-100 rounded text-sm">
              {marks.every(m => m) ? 'Desmarcar' : 'Marcar'} todas
            </button>
            <button onClick={() => setMarks(new Array(total).fill(false))}
              className="px-3 py-1.5 bg-red-50 text-red-600 rounded text-sm">Limpiar</button>
          </div>
          <div className="bg-blue-50 rounded-lg p-3">
            <div className="flex justify-between">
              <span>Progreso: {cnt} de {total}</span>
              <span className="text-lg font-bold text-blue-700">{cnt}</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
              <div className="bg-blue-600 h-2 rounded-full" style={{ width: (cnt / total) * 100 + '%' }} />
            </div>
          </div>
          <div className="flex gap-3">
            <button onClick={() => onScoreCalculated(cnt, marks)}
              className="flex-1 py-3 bg-green-600 text-white rounded-lg font-medium">Aplicar puntaje ({cnt})</button>
...
export const CLAInterface = React.memo(function CLAInterface({ onComplete, onUpdatePatient, patientAge }: CLAInterfaceProps) {
  ...
  const flip = () => {
    ...
    i.onload = () => {
      ...
      if (x) { x.translate(c.width, 0); x.scale(-1, 1); x.drawImage(i, 0, 0) }
      setImg(c.toDataURL('image/jpeg', 0.85))
    }
  }
  ...
  const applyManual = () => {
    const c = parseInt(mcnt, 10)
    if (!isNaN(c) && c >= 0 && c <= max) setRaw(c.toString())
  }
  ...
  return (
    <div className="space-y-4">
        ...
        {!img ? (
          ...
          <div>
            ...
            <div className="mt-4 pt-3 border-t">
              ...
              <div className="flex gap-2">
                <input type="number" value={mcnt} onChange={e => setMcnt(e.target.value)} min="0" max={max}
                  placeholder="Cantidad" className="flex-1 px-3 py-1.5 text-sm border rounded" />
                <button onClick={applyManual} className="px-4 py-1.5 bg-gray-600 text-white text-sm rounded">Aplicar</button>
              </div>
            </div>
          </div>
        )}
      ...
      {grid && img && (
        <ScoringGrid imageData={img} patientAge={patientAge}
          onScoreCalculated={s => { setRaw(s.toString()); setGrid(false) }}
