...
export const RVInterface = React.memo(function RVInterface({ onComplete, onUpdatePatient, patientAge }: RVInterfaceProps) {
  ...
  useEffect(() => { setSelectedAnswers([]) }, [currentIndex])

  useEffect(() => {
    if (currentItem && !isCompleted) {
      const imageName = isPractice ? `rv${currentItem.num.toString().toLowerCase()}.png` : `rv${String(currentItem.num).padStart(3, '0')}.png`
      onUpdatePatientRef.current({
        type: 'wisc5_rv', itemNum: currentItem.num, imagePath: `/wisc5/rv/${imageName}`, isPractice: currentItem.isPractice
      })
    }
  ...
  const getCurrentImagePath = (): string => {
    ...
    const imageName = isPractice ? `rv${currentItem.num.toString().toLowerCase()}.png` : `rv${String(currentItem.num).padStart(3, '0')}.png`
    return `/wisc5/rv/${imageName}`
  }

  const toggleAnswer = (num: number) => {
    if (scores[currentItem.num] !== undefined || timeEnded) return
    setSelectedAnswers(prev => prev.includes(num) ? prev.filter(n => n !== num) : prev.length < 3 ? [...prev, num] : prev)
  }
  ...
  const getNextItemIndex = (currentItemNum: number | string, currentScore: number): { nextIndex: number; updatedScores: Record<string | number, number> } => {
    ...
    if (!hasBacktrack) {
      ...
      while (nextIdx < RV_ITEMS.length && updatedScores[RV_ITEMS[nextIdx].num] !== undefined) nextIdx++
    ...
    if (backtrackMode) {
      if (currentScore === 1) {
        ...
        if (prevItem >= 1) return { nextIndex: RV_ITEMS.findIndex(i => i.num === prevItem), updatedScores }
      } else {
        setConsecutiveSuccessesInBacktrack(0)
        let prevItem = numericItem - 1
        while (prevItem >= 1 && updatedScores[prevItem] !== undefined) prevItem--
  ...
  if (isCompleted) {
    ...
    return (
      <div className="bg-green-50 rounded-lg p-4 text-center">
        ...
        <p className="text-sm text-green-600 mt-1">Puntaje total: {total} / {maxScore}{bonusApplied && <span className="ml-2 text-blue-600">(incluye +{bonus} bonus)</span>}</p>
      </div>
    )
  }
    ...
    <div className="space-y-4">
      <div className="bg-gray-50 rounded-lg p-3">
        <div className="flex justify-between items-center mb-1">
          <span className="text-gray-600 text-sm">Rompecabezas Visuales</span>
          <span className="text-3xl font-bold text-gray-800">{isPractice ? 'Práctica' : currentItem.num}</span>
        </div>
        <div className="flex justify-between text-xs text-gray-500 mb-1"><span>Ítem actual</span><span>de {RV_ITEMS.filter(i => !i.isPractice).length}</span></div>
        <div className="h-2 bg-gray-200 rounded-full overflow-hidden"><div className="h-full bg-blue-500 rounded-full" style={{ width: `${(Object.keys(scores).filter(k => k !== 'E' && k !== 'P').length / RV_ITEMS.filter(i => !i.isPractice).length) * 100}%` }} /></div>
        ...
        {bonusApplied && <p className="text-xs text-blue-600 mt-1">✓ Bonus de +{calculateBonusPoints()} pts aplicado</p>}
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4">
        ...
        <img src={getCurrentImagePath()} alt={`RV ${currentItem.num}`} className="mx-auto max-h-40 object-contain border border-gray-200 rounded-lg" onError={(e) => { e.currentTarget.src = '/placeholder-image.png' }} />
        <div className="mt-2 p-2 bg-blue-50 rounded text-center">
          <p className="text-sm text-blue-700"><strong>Respuestas correctas: {currentItem.correctAnswers.join(', ')}</strong></p>
        </div>
      </div>

      {scores[currentItem.num] === undefined && !timeEnded && (
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <p className="text-sm text-gray-600 mb-3">Selecciona las 3 respuestas del evaluado:{isPractice && <span className="ml-2 text-xs text-gray-400">(no suma puntos)</span>}</p>
          <div className="grid grid-cols-3 gap-3 mb-4">
            {[1, 2, 3, 4, 5, 6].map(num => (
              <button key={num} onClick={() => toggleAnswer(num)}
                className={`py-4 rounded-lg text-xl font-bold transition-all ${selectedAnswers.includes(num) ? 'bg-blue-600 text-white shadow-md scale-105' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>{num}</button>
            ))}
          </div>
          <button onClick={handleVerify} disabled={selectedAnswers.length !== 3}
            className={`w-full py-3 rounded-lg font-medium transition-colors ${selectedAnswers.length === 3 ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-gray-300 text-gray-500 cursor-not-allowed'}`}>
            {selectedAnswers.length === 0 ? 'Selecciona 3 opciones' : selectedAnswers.length < 3 ? `Faltan ${3 - selectedAnswers.length} opciones` : 'Verificar respuesta'}
          </button>
        </div>
      )}
      ...
      {timeEnded && scores[currentItem.num] === undefined && (
        <div className="bg-yellow-50 rounded-lg p-3 text-center border border-yellow-200">
          ...
          <button onClick={handleTimeExpired} className="px-4 py-2 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 text-sm">Registrar como incorrecto</button>
        </div>
      )}
      ...
      {scores[currentItem.num] !== undefined && (
        <div className={`rounded-lg p-3 text-center ${scores[currentItem.num] === 1 ? 'bg-green-50' : 'bg-red-50'}`}>
          <p className={`text-sm ${scores[currentItem.num] === 1 ? 'text-green-700' : 'text-red-700'}`}>
            ...
            {!isPractice && scores[currentItem.num] === 0 && <span className="ml-2">(Correctas: {currentItem.correctAnswers.join(', ')})</span>}
      ...
      <div className="bg-gray-50 rounded-lg p-3">
        <p className="text-sm text-gray-600">
          Puntaje bruto acumulado: {Object.values(scores).reduce((a, b) => a + b, 0)} / {RV_ITEMS.filter(i => !i.isPractice).length}
        </p>
      </div>
    </div>
  )
